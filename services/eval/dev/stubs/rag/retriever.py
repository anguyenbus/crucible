...
class SemanticRetriever:
    ...
    def retrieve(self, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        ...
        with tracer.start_as_current_span(
            ...
            for i, chunk in enumerate(chunks):
                span.set_attribute(f"retrieval.documents.{i}.document.id", chunk.get("doc_id", ""))
                span.set_attribute(
    ...
    def _retrieve_internal(self, query: str, top_k: int) -> list[dict[str, Any]]:
        """
        Retrieve chunks internally without span emission.
        ...
        if results and results.get("ids") and results["ids"][0]:
            for rank, chunk_id in enumerate(results["ids"][0]):
                # Extract metadata
                metadata = results["metadatas"][0][rank] if results.get("metadatas") else {}
                ...
                distance = results["distances"][0][rank] if results.get("distances") else 0.0
