...
class QueryRequest(BaseModel):
    ...
    trace_context: dict[str, Any] | None = Field(default=None, description="Trace context metadata")


class QueryResponse(BaseModel):
    """Response model for POST /query."""

    retrieved_contexts: list[str] = Field(..., description="Retrieved chunk texts in rank order")
    response: dict[str, str] = Field(..., description="Generated response with text field")
...
async def _query_with_span(
    ...
    with tracer.start_as_current_span(
        ...
        retrieved_contexts = [chunk.get("text", "") for chunk in result.get("retrieved_chunks", [])]
