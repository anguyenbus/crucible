...
def test_constructor_injects_metrics_without_importing_config() -> None:
    ...
    # then restore the original sys.modules state in a finally so this test does
    # not leak a popped module into later tests (a fresh re-import would otherwise
    # create a second module object monkeypatch.setattr targets cannot reach).
    _saved = sys.modules.pop("app.deepeval.bedrock_provider", None)
    try:
        ...
        if _saved is not None:
            sys.modules["app.deepeval.bedrock_provider"] = _saved
