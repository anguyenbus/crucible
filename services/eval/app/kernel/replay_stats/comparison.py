...
def paired_comparison(
    ...
    candidate_error_rate = candidate_errors / candidate_total if candidate_total > 0 else 0.0
    baseline_error_rate = baseline_errors / baseline_total if baseline_total > 0 else 0.0
...
def _wilcoxon_test(
    ...
    """
    # NOTE: scipy is a core kernel dependency. The previous `except ImportError`
    # sign-test fallback was DELETED in Phase 2: a missing scipy must surface as
    # an ImportError at module import, never silently produce a different
    # statistic from a divergent code path.
