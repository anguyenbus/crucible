...
- ``run_golden_set(...) -> RunResult`` -- the default OTLP-tracing loop. It
  iterates the dataset, queries the injected ``RagAdapter``, computes metrics via
  the injected evaluator, and COLLECTS the per-query rows + a summary into a pure
  ``RunResult`` dataclass. It performs NO file I/O: there is no
  ``Path("results/...")`` / ``Path("data/...")`` reachable from this module. The
  CLI shell receives the ``RunResult`` and persists it via
  ``crucible.service.metrics.csv_writer``.
- ``run_phoenix_native(...)`` -- the ``--phoenix-native`` path. It has a different
  dependency set + return shape and delegates to
  ``crucible.service.phoenix.experiments``.

All infra (evaluator, phoenix adapter, RAG adapter, metrics) is BUILT IN THE
SHELL and injected; the library never builds boto/openai/phoenix clients itself.
"""
...
def run_golden_set(
    *,
    ...
    """
    Run the default golden-set evaluation loop and collect a ``RunResult``.
    ...
    Args:
        ...
        phoenix: Optional Phoenix adapter for OTLP spans (best-effort, no-op when
            ``None``).
    ...
    eval_run_ctx = (
        phoenix.eval_run_span(run_name="golden-set", num_questions=num_queries, metadata={})
    ...
    with eval_run_ctx:
        for query_id, query_text, relevant_passage_id, gold_answer in dataset_list:
            query_ctx = phoenix.rag_query_span(question=query_text) if phoenix is not None else None
            if query_ctx is not None:
                _ = query_ctx.__enter__()
            try:
                ...
                if query_ctx is not None:
                    _ = query_ctx.__exit__(None, None, None)
