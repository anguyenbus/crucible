...
class StubConfig(BaseModel):
    ...
    chunk_size: int = Field(
        ..., ge=MIN_CHUNK_SIZE, le=MAX_CHUNK_SIZE, description="Target chunk size in characters"
    )
    chunk_overlap: int = Field(..., ge=0, description="Overlap between chunks in characters")
    embedding_model: str = Field(
        default=DEFAULT_EMBEDDING_MODEL, description="HuggingFace model identifier for embeddings"
    )
    port: int = Field(
        ..., ge=MIN_PORT, le=MAX_PORT, description="HTTP port for this service instance"
    )
    corpus_path: Path = Field(
        ..., description="Path to document corpus (relative to YAML or absolute)"
    )
    phoenix_endpoint: str | None = Field(
        default="http://localhost:6006", description="Phoenix endpoint for distributed tracing"
    )
    export_spans: bool = Field(
        default=True, description="Whether to export OpenInference spans to Phoenix"
    )
    retrieval_backend: str = Field(
        default="chromadb", description="Retrieval backend: 'chromadb', 'faiss', or 'zvec'"
    )

    @field_validator("chunk_overlap")
    @classmethod
    def validate_overlap_less_than_chunk_size(cls, v: int, info: object) -> int:
