# Eval Refactor Plan v2 — Shape It for the `services/eval/` Migration

**Branch:** `bedrock` → refactor branch `restructure/eval-shape`
**Objective:** refactor eval *in place* so the later move into `genai-backend/services/eval/app/` is a directory copy plus one mechanical, machine-verified import rewrite — no redesign, no untangling, no surprises on migration day.
**Success criterion (binding):** a CI job (`rehearse-migration`) copies eval into a **skeletal but real** `services/eval/` destination on every PR, runs `uv sync --frozen` against the destination's own lockfile, asserts zero surviving `app.*` references, import-smokes every module, and runs the test suite. Green = the migration is continuously proven, not merely planned.
...
| 3 | Schema packaging deferred to Phase 5; "mechanical move" | `rag_adapter.py:90` hardcodes `Path("services/eval/app/contracts/…")` (CWD-relative). Fixed in **Phase 2** when the validator moves: injectable schema path, `importlib.resources` default | The copy is not mechanical until this lands; the backend swap (next row) detonates it |
| 4 | hatchling implicit | **hatchling will not be used.** Backend swap to `uv_build` is a Phase-0 commit; rehearsal validates packaging via `uv sync` | Team decision; hatchling's free inclusion of `services/eval/app/contracts/*.json` disappears with it |
...
eval's internal layout is refactored to mirror it one-to-one, under two top-level subpackages with a strictly one-way dependency:

```
app.kernel.*    →  copies to  app/kernel/*      (pure; no infra, no env reads, no network)
app.*   →  copies to  app/<dir>/*       (deepeval/, phoenix/, datasets/, metrics/, runners/, config)
```

Everything that will **not** migrate (demo stubs, CLI shells, local docker-compose) is quarantined under `dev` where the copy never touches it. The import rewrite is one rule, **verified after the fact rather than trusted**:

```
app.kernel.X   →  app.kernel.X
app.X  →  app.X
…then: assert no string "app." survives anywhere in the copied tree.
```

Two import disciplines make the rewrite mostly unnecessary: (1) **within** a subpackage, relative imports only; (2) **across** the kernel/service boundary, absolute `app.kernel.…` imports, service-side only. Both are enforced by the import linter; the post-sed assertion (§4 Phase 6) catches anything the disciplines or the sed miss — including string-literal module paths in tests.
...
| 0.1 | **Python version.** eval pins `>=3.13` (Dockerfile: `python:3.13-slim`); confirm the monorepo `services/*` toolchain minor. | align eval to the monorepo's exact minor | The one thing that can hard-block the copy. |
...
| 0.3 | **Lint/format parity.** Copy the monorepo ruff/format settings into eval now; land as one isolated formatting commit before any structural change. | adopt monorepo config | Otherwise migration day buries the real diff under a reformat. |
...
**0.5 — the locked protocol** (kernel `interfaces.py`; implemented in the monorepo, no-op default in eval):
...
**Exit criteria:** decisions recorded at the top of eval's README; ruff-config commit and `uv_build` commit landed (separately) before any structural change.

---

## 2. Target end-state layout of eval
...
│   │   │                           #     The CWD-relative Path("services/eval/app/contracts/…") at
...
│       ├── cli/                    #   eval / eval-rag / eval-replay / generate-spans shells +
...
1. `kernel/` may import: stdlib, `pydantic`, `jsonschema`, `scipy`, `deepeval`, `beartype`. It may **not** import `boto3`, `phoenix`/`arize`, `dotenv`, `app`, `dev`, **read** `os.environ`, or **write** it — except the single allowlisted telemetry line in `kernel/rag_metrics/__init__.py` (0.7).
2. `service/` may import `kernel` (absolute) and third-party infra; never `dev`.
...
type = "forbidden"
source_modules = ["app.kernel", "app"]
forbidden_modules = ["dev"]
```

**Acceptance:** linter green; `pip install eval` (no extras) imports cleanly with no chromadb/sentence-transformers present; CLIs run with `[demo]`.
...
3. Move `schema_validator.py` into `kernel/validation/` with the **injectable schema path** API: `validate(payload, schema=..., schema_dir: Path | None = None)`, default resolving via `importlib.resources` into the packaged `contracts/`. Eliminate `rag_adapter.py:90`'s `Path("services/eval/app/contracts/…")` in the same PR. Add the no-CWD-path grep-gate (§2 rule 5).
...
6. Activate the kernel-purity import-linter contract **plus** the grep-gate: `os.environ`, `load_dotenv`, `boto3`, `phoenix` forbidden under `services/eval/app/kernel/`, with the single-line 0.7 allowlist.
...
2. Extract runner logic out of `__main__` shells into plain functions: `run_golden_set(dataset, adapter, provider, config, claims: ClaimStore | None = None) -> RunResult` (0.5 protocol — claim before judge calls when provided, finalize after result write; `None` = no idempotency layer, eval-standalone behaviour).
...
1. The telemetry opt-out fires at the **two** 0.7 sites only — `kernel/rag_metrics/__init__.py` and `service/deepeval/__init__.py` — each before its own first `import deepeval`. The package root `eval/__init__.py` writes nothing.
2. **Two subprocess regression tests**, one per import path: fresh interpreter, assert `"deepeval" not in sys.modules`, then (a) `import app.kernel.rag_metrics` and (b) `import app.deepeval`; assert the env var is set in both cases before deepeval landed in `sys.modules`. (v1 tested only the service path — the kernel path is the one its own Phase-2 acceptance exercises.)
3. `load_dotenv()` moves to the legitimate entrypoints only: `local/cli/*`, and `service/config.py` behind an explicit `from_dotenv=True`. Library import of any eval module mutates nothing beyond the 0.7 sites.
4. Note for the record (not eval's to fix): the env var governs DeepEval analytics; deepeval also vendors `sentry-sdk`/`posthog`. The monorepo deployment's egress policy is the hard telemetry perimeter (0.7 backstop).

**Acceptance:** `python -c "import app"` performs zero env writes (tested); both subprocess tests green.
...
- Verify `uv_build` packages `contracts/*.json` as data files (explicit include config) and that an install **from the built wheel** — not just an editable/path install — passes the kernel tests, exercising the `importlib.resources` path for schemas. Then delete `services/eval/app/contracts/` (the 3 vendored duplicates; byte-identical today, verified). Top-level `contracts/` (4 schemas, `schema_version` present in all 4 — verified) is the sole source and later moves untouched to `genai-backend/contracts/schemas/`.
...
**6a. Build the skeleton now** (one-time, small): `genai-backend/services/eval/` — or, until monorepo write access exists, a `skeleton/services/eval/` directory inside eval mirroring it — containing: `app/__init__.py` (a real package, not an implicit namespace — matching how every other `services/*` app is laid out), `pyproject.toml` in house style (eval's deps verbatim + `bedrock`/`phoenix` extras promoted per the §5 runbook; `doc-bench` pin commented until wired), a generated per-service `uv.lock`, and `tests/`. This turns the rehearsal from "simulate a guessed destination" into "copy into the real one."
...
rm -rf "$DEST/app/kernel"; cp -r services/eval/app/kernel "$DEST/app/kernel"
for d in services/eval/app/service/*/; do
  rm -rf "$DEST/app/$(basename "$d")"; cp -r "$d" "$DEST/app/$(basename "$d")"
done
cp services/eval/app/service/config.py "$DEST/app/config.py"
...
grep -rl "eval\." "$DEST/app" "$DEST/tests" | xargs -r sed -i \
  -e 's/eval\.kernel/app.kernel/g' \
  -e 's/eval\.service\./app./g' \
  -e 's/from eval\.service import/from app import/g' \
  -e 's/from eval import/from app import/g'
...
if grep -rn "eval" "$DEST/app" "$DEST/tests"; then
  echo "FATAL: surviving app.* references after rewrite"; exit 1
fi
...
2. `services/eval/pyproject.toml` already carries eval's deps (skeleton, 6a); add the `doc-bench==Y` pin; `uv lock` and commit the per-service lockfile.
...
6. Archive the eval repo with a tombstone README pointing at `services/eval/`.
...
## 6. Deliberately NOT built in eval (and why)

| Not built | Why it stays out |
|---|---|
| Token bucket, `ClaimStore` implementation, REST API, S3 span reader | Infra-bound (Redis/RDS/DynamoDB/ALB/IRSA); building against local fakes would be thrown away. Eval defines the **protocols** (`RateLimiter`, `ClaimStore` — now two-phase per 0.5) so they slot in without touching migrated code. |
| `parse_run` / comparator integration | doc-bench's job (external pinned wheel); wiring is monorepo work. |
| Renaming the package to `app` inside eval | Breaks standalone installability during the transition window; the rehearsed, *asserted* rewrite is cheaper than a broken interim repo. |
...
9. **Rehearsal green** — including the post-sed zero-`app.*` assertion, `uv sync --frozen` against the destination lock, and the all-modules import smoke. Required job.
...
- [ ] `pip install eval` → zero env mutations on import of the root package; the two 0.7 sites tested by subprocess; no demo deps in direct core; kernel importable standalone and from the built wheel.
