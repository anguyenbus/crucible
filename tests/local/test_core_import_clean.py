...
This is the REAL Phase 1 enforcement of the ``crucible.local`` quarantine. The
import-linter contracts are vacuous in Phase 1 (``crucible.kernel`` /
``crucible.service`` do not exist yet), so this subprocess/``sys.modules`` check
...
# Phase 2 and must never reach into crucible.local.stubs.
_CORE_MODULE = "crucible.kernel.interfaces"
...
def test_core_import_pulls_in_no_demo_dependencies():
    ...
    assert leaked == [], (
        ...
        f"{leaked}. The crucible.local quarantine has been breached."
    )
