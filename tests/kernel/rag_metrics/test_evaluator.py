...
- ``crucible.kernel.rag_metrics.evaluator.DeepEvalEvaluator`` now receives an
...
- ``crucible.kernel.rag_metrics.samples.transform_to_deepeval_sample`` produces
...
def test_constructor_injects_metrics_without_importing_config() -> None:
    ...
    sys.modules.pop("crucible.service.deepeval.bedrock_provider", None)
    ...
    assert "crucible.service.deepeval.bedrock_provider" not in sys.modules
