...
from crucible.service.deepeval.bedrock_provider import DEFAULT_JUDGE_MODEL, get_deepeval_config

# Repo root is three parents up from this file: tests/service/deepeval/<this file>.
_EVAL_CONFIG_PATH = Path(__file__).resolve().parents[3] / "eval_config.yaml"

# These tests assert the SHAPE of the repo-root eval_config.yaml -- a crucible-repo
...
# config-shape assertions remain a crucible-repo concern.
if not _EVAL_CONFIG_PATH.exists():
    pytest.skip(
        "eval_config.yaml not present at the project root (repo-root config-shape "
        "tests do not apply outside the crucible checkout)",
